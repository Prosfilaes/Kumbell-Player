...
package Move_Book is
   ...
   procedure Add_Move (f: Ada.Text_IO.File_Type; b : Game_State; depth : Integer) with
     Pre  => Is_Legal_Board (b) and then not Game_Over (b);
   procedure Add_Move (f: Ada.Text_IO.File_Type; b : Game_State) with
     Pre  => Is_Legal_Board (b) and then not Game_Over (b);
